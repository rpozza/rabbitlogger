...
JSON_CONFIG_LOCATION = "/usr/local/bin/rabbitlogger/myconfig.json"
...
class SwiftConsumer(object):
    ...
    QUEUE = 'swift-storage'
    ROUTING_KEY = None
    CONTAINER = 'sensordump'
    PREV_PATHNAME = None
    PREV_OBJNAME = None
    FILE_WINDOWSIZE = 15

    def __init__(self, amqp_url, sw_url, sw_vers, sw_user, sw_pswd, sw_tena):
        ...
        self._url = amqp_url
        self._swift = swiftclient.Connection(authurl=sw_url, user=sw_user, key=sw_pswd,
                                             tenant_name=sw_tena, auth_version=sw_vers)
        resp_headers, containers = self._swift.get_account()
        # Check if present or create a new container
        if not containers:
            # create container for dump data
            self._swift.put_container(self.CONTAINER)
        else:
            # check it exists
            found = 0
...
def main():
    ...
    SW_URL = 'http://' + config_data["OpStController"] + ':5000/v2.0/'
    SW_VERS = 2
    SW_USER = config_data["OpStObjUsername"]
    SW_PSWD = config_data["OpStObjPassword"]
    SW_TENA = config_data["OpStObjTenantname"]
    consumer = SwiftConsumer(AMQP_URL, SW_URL, SW_VERS, SW_USER, SW_PSWD, SW_TENA)
    try:
        consumer.run()
